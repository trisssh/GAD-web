...
export default function PublicDocumentTable({
    ...
    const fetchDocuments = async () => {
        try {
            setLoading(true);
            const response = await axios.get(`showpublicdocuments?type=${documentType}`);
    ...
    const filteredDocuments = useMemo(() => {
        return documents.filter((doc) => {
            const matchesDocumentType = doc.document_type === documentType;
            const matchesSector =
                !selectedSector ||
                doc.sector_name?.toLowerCase() === selectedSector.toLowerCase();
            const matchesSubsector =
                !selectedSubsector ||
                doc.sub_sector_name?.toLowerCase() ===
                    selectedSubsector.toLowerCase();
            const matchesBarangay =
                !selectedBarangay ||
                doc.barangay_name?.toLowerCase() === selectedBarangay.toLowerCase();
            const matchesGender =
                !selectedGender ||
    ...
    }, [documents, documentType, selectedSector, selectedSubsector, selectedBarangay, selectedGender, keyword]);
