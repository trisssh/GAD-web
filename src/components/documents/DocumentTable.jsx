...
import EditDocumentModal, { AddDocumentModal } from "../documents/Modal";
...
export default function DocumentTable({
  ...
  const fetchDocuments = async () => {
    try {
      setLoading(true);
      const response = await axios.get("showdocuments");
  ...
  const handleDelete = async (id) => {
    ...
    if (confirm.isConfirmed) {
      try {
        const response = await axios.post("destroydocuments", { id });
  ...
  return (
    <div className="overflow-x-auto bg-white rounded-xl shadow-md p-4 mt-6">
      ...
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-semibold text-gray-800">Document Data</h2>
        <button
...
function DocumentTableRow({ entry, onDelete, onEdit }) {
